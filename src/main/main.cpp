...
static void run_feather_benchmark(const fs::path& data_dir) {
    ...
    if (!fs::exists(script)) script = "../scripts/convert_to_feather.py";
...
int main(int argc, char* argv[]) {
    ...
    for (auto& s : streams) total_parsed += s.size();
    ...
    dispatcher.set_snapshot_callback(
        [&snap_count](uint64_t iid, const LimitOrderBook& lob, uint64_t n) {
            ++snap_count;
            if (snap_count <= 2) { // only print first 2 to avoid noise
    ...
    // Dispatcher thread
    std::thread disp_thread([&]{ dispatcher.run(); });
    ...
    for (std::size_t num_workers : {2u, 4u}) {
        ...
        std::thread shard_thread([&]{ sharded.run(); });

        {
            auto streams_copy = streams;
            FlatMerger merger(std::move(streams_copy));
            MarketDataEvent ev;
            while (merger.next(ev)) sharded_queue.push(ev);
