...
TEST(Matcher, CancelUnknownIdReturnsUnknownOrder) {
    bt::PessimisticQueueModel qm;
    bt::Matcher m(qm);
    EXPECT_EQ(m.cancel(/*id=*/999, /*now=*/0),
              bt::Matcher::CancelResult::UnknownOrder);
    EXPECT_EQ(m.resting_count(), 0u);
}

TEST(Matcher, CancelOfFilledOrderReturnsUnknownOrder) {
    bt::PessimisticQueueModel qm;
    bt::Matcher m(qm);
    const auto book = book_with(100, 101);
    auto r = m.submit(/*id=*/1, bt::Side::Buy, 99, 5, book, 0);
    ...
    auto fills = m.on_trade(bt::Trade{ 1, bt::Side::Sell, 99, 5 }, 1);
...
TEST(Matcher, OnTradePartialFillCappedByTradeAmount) {
    ...
    EXPECT_TRUE(m.has_order(1));  // still has 6 left
...
TEST(Matcher, QueueErosionThreadsThroughFifoOrdersAtSameLevel) {
    ...
    EXPECT_EQ(fills[0].qty, 5);
    EXPECT_FALSE(m.has_order(1));
    EXPECT_TRUE(m.has_order(2));  // B still queued
}
